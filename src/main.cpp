...
namespace {
	...
	void TestReference() {
		...
		const auto a3_value = sheet->GetCell("A3"_pos)->GetValue();
		ASSERT(std::holds_alternative<double>(a3_value));
		ASSERT_EQUAL(std::get<double>(a3_value), 15);
		const auto c2_value = sheet->GetCell("C2"_pos)->GetValue();
		ASSERT(std::holds_alternative<double>(c2_value));
		ASSERT_EQUAL(std::get<double>(c2_value), 5);
	}
	void TestReferenceEmpty() {
		auto sheet = CreateSheet();
		sheet->SetCell("A2"_pos, "3");
		sheet->SetCell("C2"_pos, "=A3/A2");
		const auto c2_value = sheet->GetCell("C2"_pos)->GetValue();
		ASSERT(std::holds_alternative<double>(c2_value));
		ASSERT_EQUAL(std::get<double>(c2_value), 0);
		std::stringstream ss;
	...
	void TestDiv0() {
		...
		sheet->SetCell("C2"_pos, "=A3/A2");
		const auto c2_value = sheet->GetCell("C2"_pos)->GetValue();
		ASSERT(std::holds_alternative<FormulaError>(c2_value));
		ASSERT_EQUAL(std::get<FormulaError>(c2_value).ToString(), "#DIV/0!");
	}
	void TestValueError() {
		auto sheet = CreateSheet();
		...
		const auto c2_value = sheet->GetCell("C2"_pos)->GetValue();
		ASSERT(std::holds_alternative<FormulaError>(c2_value));
		ASSERT_EQUAL(std::get<FormulaError>(c2_value).ToString(), "#VALUE!");
	}
	void TestCircularException() {
		auto sheet = CreateSheet();
		sheet->SetCell("A2"_pos, "3");
		sheet->SetCell("C2"_pos, "=A3/A2");
...
int main() {
	...
	RUN_TEST(tr, TestReferenceEmpty);
	RUN_TEST(tr, TestDiv0);
	RUN_TEST(tr, TestValueError);
	RUN_TEST(tr, TestCircularException);
	return 0;
}